  ...
  async scrapeSchemes() {
    try {
      ...
      try {
        ...
        // Updated selectors based on actual myScheme.in HTML structure (verified 2026-04-26)
        const schemeElements = $('div[role="article"]');

        schemeElements.each((index, element) => {
          try {
            const $el = $(element);

            // Extract data using verified selectors from myScheme.in HTML
            const title = $el.find('h2 a span').first().text().trim();
            const description = $el.find('span.line-clamp-2 span').text().trim();
            const link = $el.find('h2 a').first().attr('href') || '';
            
            // Extract tags/categories from tag buttons
            const tags = [];
            $el.find('div[role="button"] span').each((i, el) => {
              const tag = $(el).text().trim();
              if (tag) tags.push(tag);
            });
            const category = tags.length > 0 ? tags[0] : 'Government Scheme';
